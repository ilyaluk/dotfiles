...
&& ret=0
    ...
    (bisect)
        words=($line[1] "${words[@]}")
        (( CURRENT += 1 ))
        curcontext="${curcontext%:*:*}:jj-bisect-command-$line[1]:"
        case $line[1] in
            (run)
_arguments "${_arguments_options[@]}" : \
'*-r+[Range of revisions to bisect]:REVSETS:_default' \
'*--range=[Range of revisions to bisect]:REVSETS:_default' \
'--command=[Command to run to determine whether the bug is present]:COMMAND:_default' \
'-R+[Path to repository to operate on]:REPOSITORY:_files -/' \
'--repository=[Path to repository to operate on]:REPOSITORY:_files -/' \
'--at-operation=[Operation to load the repo at]:AT_OPERATION:_default' \
'--at-op=[Operation to load the repo at]:AT_OPERATION:_default' \
...
(index-changed-paths)
_arguments "${_arguments_options[@]}" : \
'-n+[Limit number of revisions to index]:LIMIT:_default' \
'--limit=[Limit number of revisions to index]:LIMIT:_default' \
'-R+[Path to repository to operate on]:REPOSITORY:_files -/' \
'--repository=[Path to repository to operate on]:REPOSITORY:_files -/' \
'--at-operation=[Operation to load the repo at]:AT_OPERATION:_default' \
'--at-op=[Operation to load the repo at]:AT_OPERATION:_default' \
...
&& ret=0
    ...
    (object)
        words=($line[1] "${words[@]}")
        (( CURRENT += 1 ))
        curcontext="${curcontext%:*:*}:jj-debug-object-command-$line[1]:"
        case $line[1] in
            (commit)
_arguments "${_arguments_options[@]}" : \
'-R+[Path to repository to operate on]:REPOSITORY:_files -/' \
...
'(-s --summary --stat --types --name-only --git --color-words --tool)-T+[Render each file diff entry using the given template]:TEMPLATE:_default' \
'(-s --summary --stat --types --name-only --git --color-words --tool)--template=[Render each file diff entry using the given template]:TEMPLATE:_default' \
...
'-h[Print help (see more with '\''--help'\'')]' \
'--help[Print help (see more with '\''--help'\'')]' \
'*::paths -- Edit only these paths (unmatched paths will remain unchanged):_files' \
&& ret=0
;;
(duplicate)
_arguments "${_arguments_options[@]}" : \
...
(evolog)
_arguments "${_arguments_options[@]}" : \
'*-r+[Follow changes from these revisions]:REVSETS:_default' \
'*--revisions=[Follow changes from these revisions]:REVSETS:_default' \
...
'-T+[Render each file metadata using the given template]:TEMPLATE:_default' \
'--template=[Render each file metadata using the given template]:TEMPLATE:_default' \
'-R+[Path to repository to operate on]:REPOSITORY:_files -/' \
'--repository=[Path to repository to operate on]:REPOSITORY:_files -/' \
'--at-operation=[Operation to load the repo at]:AT_OPERATION:_default' \
'--at-op=[Operation to load the repo at]:AT_OPERATION:_default' \
...
&& ret=0
    ...
    (gerrit)
        words=($line[1] "${words[@]}")
        (( CURRENT += 1 ))
        curcontext="${curcontext%:*:*}:jj-gerrit-command-$line[1]:"
        case $line[1] in
            (upload)
_arguments "${_arguments_options[@]}" : \
...
(show)
_arguments "${_arguments_options[@]}" : \
'-T+[Render the operation using the given template]:TEMPLATE:_default' \
...
'--git[Show a Git-format diff]' \
'--color-words[Show a word-level diff with changes indicated only by color]' \
'(-s --summary --stat --types --name-only --git --color-words --tool --context -w --ignore-all-space -b --ignore-space-change)--no-patch[Do not show the patch]' \
'-w[Ignore whitespace when comparing lines]' \
'--ignore-all-space[Ignore whitespace when comparing lines]' \
'(-w --ignore-all-space)-b[Ignore changes in amount of whitespace when comparing lines]' \
'(-w --ignore-all-space)--ignore-space-change[Ignore changes in amount of whitespace when comparing lines]' \
...
'-h[Print help (see more with '\''--help'\'')]' \
'--help[Print help (see more with '\''--help'\'')]' \
'*::paths -- Files matching any of these filesets are put in the selected changes:_files' \
&& ret=0
;;
(squash)
_arguments "${_arguments_options[@]}" : \
...
'*::args -- Arguments to pass to the external command:_files' \
&& ret=0
;;
(gc)
_arguments "${_arguments_options[@]}" : \
...
'-h[Print help (see more with '\''--help'\'')]' \
'--help[Print help (see more with '\''--help'\'')]' \
':destination -- Where to create the new workspace:_files -/' \
&& ret=0
;;
(forget)
_arguments "${_arguments_options[@]}" : \
...
(list)
_arguments "${_arguments_options[@]}" : \
'-T+[Render each workspace using the given template]:TEMPLATE:_default' \
'--template=[Render each workspace using the given template]:TEMPLATE:_default' \
'-R+[Path to repository to operate on]:REPOSITORY:_files -/' \
'--repository=[Path to repository to operate on]:REPOSITORY:_files -/' \
'--at-operation=[Operation to load the repo at]:AT_OPERATION:_default' \
'--at-op=[Operation to load the repo at]:AT_OPERATION:_default' \
...
'backout:(deprecated; use \`revert\`) Apply the reverse of given revisions on top of another revision' \
...
'index-changed-paths:Build changed-path index' \
